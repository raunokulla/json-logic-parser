...
Parser.prototype.assignment = function () {
  ...
  if (left instanceof SymbolNode && this.peek('=')) {
    this.consume();
    return new AssignmentNode(left.var, this.assignment());
  }
  return left;
};
...
Parser.prototype.symbol = function () {
  ...
  if (current.type === tokenType.SYMBOL) {
    ...
    return this.functionCall(symbol);
  }
  return this.string();
};
...
Parser.prototype.functionCall = function (symbolToken) {
  ...
  if (this.peek('(')) {
    ...
    var params = [];
    while ((groupStarted && this.peek(')')) || !this.peek(')') && !this.isEOF()) {
      params.push(this.assignment());
      if (this.peek(',')) {
        this.consume();
      } else if (this.peek(')') && groupStarted) {
        this.consume();
        groupStarted = false;
      }
    }
...
Parser.prototype.string = function () {
  if (this.current().type === tokenType.STRING) {
    if (this.current().value.match(/\d{4}-\d{2}-\d{2}/g)) {
      return this.consume().value;
    }
    return new SymbolNode(this.consume().value);
  }
  return this.array();
};
